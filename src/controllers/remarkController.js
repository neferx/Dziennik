...
export const addNewRemark = async (req, res) => {
  ...
  if (userExistsValidStudent) {
    if (userExistsValidTeacher) {
      ...
      return res.status(409).json({ message: 'There is no such teacher.' });
    }
  } else {
...
export const getAllRemarks = (req, res) => {
  ...
  pool.connect((err, client, release) => {
    ...
    client.query(`select * from "Remarks"`).then((resQ) => {
      release();
      remarkList = resQ.rows;
      console.log(remarkList);
      return res.status(200).json({ message: 'All remarks List', remarkList });
    });
  });
};
...
export const getAllRemarksByStudentID = async (req, res) => {
  ...
  if (userExistsValidStudent) {
    pool.connect((err, client, release) => {
      ...
      client
        .query(
          `select "studentID", "idRemarks","teacherID","remark","email","name" as "teachername","lastname" as "teacherlastname","role"
          from "Remarks" full join "User" on "Remarks"."teacherID"="User"."id" where "studentID"=${studentID}`
        )
        .then((resQ) => {
          release();
          remarkList = resQ.rows;
